...
if ( ! function_exists( 'boldthemes_breadcrumbs' ) ) {
	function boldthemes_breadcrumbs( $simple = false ) {
		...
		$home = esc_html__( 'Home', 'hotelcalifornia' );
		$home_link = home_url( '/' );
		$title = '';
		$output  = '';
		$item_prefix = '<li>';
		...
		if ( ! is_404() && ! is_home() ) {
			...
			if ( is_category() ) {
				...
				$title = esc_html__( 'Category:', 'hotelcalifornia' ) . ' ' . single_cat_title( '', false );
				...
				$output .= $item_prefix . esc_html__( 'Portfolio', 'hotelcalifornia' ) . $item_sufix;
				$output .= $item_prefix . get_the_title() . $item_sufix;
				...
				$title = esc_html__( 'Tag:', 'hotelcalifornia' ) . ' ' . single_tag_title( '', false );
				$output .= $item_prefix . $title . $item_sufix;
		  
			} else if ( is_author() ) {
			
				$title = esc_html__( 'Author:', 'hotelcalifornia' ) . ' ' . get_the_author_meta( 'display_name' );
				$output .= $item_prefix .  $title . $item_sufix;
				...
				$title = esc_html__( 'Search:', 'hotelcalifornia' ) . ' ' . get_query_var( 's' );
				$output .= $item_prefix . $title . $item_sufix;			
