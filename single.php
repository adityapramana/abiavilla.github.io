...
if ( isset( HotelCaliforniaTheme::$boldthemes_options['blog_settings_page_slug'] ) && HotelCaliforniaTheme::$boldthemes_options['blog_settings_page_slug'] != '' ) {
	HotelCaliforniaTheme::$boldthemes_page_for_header_id = boldthemes_get_id_by_slug( HotelCaliforniaTheme::$boldthemes_options['blog_settings_page_slug'] );
}
	...
	while ( have_posts() ) {
		...
		if ( has_post_thumbnail() && boldthemes_get_option( 'blog_ghost_slider' ) ) {
				...
				if ( $blog_author || $blog_date || $show_comments_number ) {
					...
					if ( $blog_date ) $meta_slider_html .= '<span class="btArticleDate">' . esc_html( date_i18n( HotelCaliforniaTheme::$boldthemes_date_format, strtotime( get_the_time( 'Y-m-d' ) ) ) ) . '</span>'; 
		...
		<?php }
			...
			if ( $blog_author || $blog_date || $show_comments_number ) {
				$meta_html .= '';
				if ( $blog_date ) $meta_html .= '<span class="btArticleDate">' . esc_html( date_i18n( HotelCaliforniaTheme::$boldthemes_date_format, strtotime( get_the_time( 'Y-m-d' ) ) ) ) . ' </span>'; 
