...
if ( get_option( 'woocommerce_enable_review_rating' ) !== 'no' ) {
	...
	if ( wc_product_sku_enabled() && ( $product->get_sku() || $product->is_type( 'variable' ) ) ) {
		...
		$sku = $product->get_sku() ? $product->get_sku() : esc_html__( 'N/A', 'hotelcalifornia' );
		$subtitle .= '<span class = "btProductSKU">' . esc_html__( 'SKU:', 'hotelcalifornia' ) . ' ' . $sku . '</span>'; 
...
echo boldthemes_get_heading_html( $supertitle, get_the_title(), $subtitle, 'large', $dash, 'btBottomDashAccentColor', '' ) ;
