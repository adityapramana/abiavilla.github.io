		...
		echo '<div class="port">';
			echo '<div class="boldCell">';
				echo '<div class="boldRow bottomSmallSpaced">';
					...
					echo boldthemes_get_heading_html( '', get_the_title(), $subheading, 'large', $dash, 'wArticleMeta btBottomDashAlternateColor', '' ) ;
					...
					if ( ( $cf != '' && count( $cf ) > $data_items_split ) || $categories_html != '' || $cf_right_html != '' ) {
						...
						if ( $categories_html != '' ) {
							echo '<dt>' . esc_html__( 'Category', 'hotelcalifornia' ) . '</dt>';
					...
					wp_link_pages( array( 
						'before'      => '<ul>' . esc_html__( 'Pages:', 'hotelcalifornia' ),
						'separator'   => '<li>',
						'after'       => '</ul>'
					));					
