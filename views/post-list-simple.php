<?php
	...
	if ( $blog_author || $blog_date || $show_comments_number ) {
		$meta_html .= '';
		if ( $blog_date  && !$blog_side_info ) $meta_html .= '<span class="btArticleDate">' . esc_html( date_i18n( HotelCaliforniaTheme::$boldthemes_date_format, strtotime( get_the_time( 'Y-m-d' ) ) ) ) . ' </span>';
		if ( $blog_date  && $blog_side_info ) $categories_html .= '<span class="btArticleDate">' . esc_html( date_i18n( HotelCaliforniaTheme::$boldthemes_date_format, strtotime( get_the_time( 'Y-m-d' ) ) ) ) . ' </span>';
		if ( $blog_author ) $categories_html .= $author_html ;
				...
				echo '<div class = "boldRow bottomSmallSpaced">';
					...
					echo '<div class="rowItem btTextLeft ' . $r_class . ' ">';
						echo '<div class="rowItemContent">';
							echo boldthemes_get_heading_html( $categories_html, '<a href="' . esc_url_raw( $permalink ) . '">' . get_the_title() . '</a>', '', 'large', $dash, 'btBottomDashAlternateColor', '' );
						echo '</div>' ;
					echo '</div>';
