...
if ( ! class_exists( 'HotelCaliforniaTheme' ) ) {
	
	class HotelCaliforniaTheme {
		...
		function __construct() {
			...
			add_action( 'admin_init', array( $this, 'boldthemes_admin_init' ) );
			add_action( 'admin_enqueue_scripts', array( $this, 'boldthemes_load_admin_style' ) );
		...
		function boldthemes_init() {
			...
			register_nav_menus( array (
				'primary' => esc_html__( 'Primary Menu', 'hotelcalifornia' ),
				'footer'  => esc_html__( 'Footer Menu', 'hotelcalifornia' )
			));
			
			// load translated strings
			load_theme_textdomain( 'hotelcalifornia', get_template_directory() . '/languages' );
			
			// date format
			if ( function_exists( 'icl_register_string' ) ) {
				icl_register_string( 'hotelcalifornia', 'Date Format', get_option( 'date_format' ) );
				HotelCaliforniaTheme::$boldthemes_date_format = icl_t( 'hotelcalifornia', 'Date Format', get_option( 'date_format' ) );
			} else {
				HotelCaliforniaTheme::$boldthemes_date_format = get_option( 'date_format' );
			}
		...
		public static $boldthemes_options;
		public static $boldthemes_page_options;
		...
		static function boldthemes_set_global_uri() {
			...
			$data .= 'window.boldthemes_text.previous = \'' . esc_html__( 'previous', 'hotelcalifornia' ) . '\';';
			$data .= 'window.boldthemes_text.next = \'' . esc_html__( 'next', 'hotelcalifornia' ) . '\';';
		...
		function boldthemes_widgets_init() {  
			...
			register_sidebar( array (
				'name' 			=> esc_html__( 'Sidebar', 'hotelcalifornia' ),
			...
			register_sidebar( array (
				'name' 			=> esc_html__( 'Header Left Widgets', 'hotelcalifornia' ),
				'id' 			=> 'header_left_widgets'
			));
			
			register_sidebar( array (
				'name' 			=> esc_html__( 'Header Right Widgets', 'hotelcalifornia' ),
			...
			register_sidebar( array (
				'name' 			=> esc_html__( 'Footer Widgets', 'hotelcalifornia' ),
				'id' 			=> 'footer_widgets',
				'before_widget' => '<div class="btBox %2$s">',
				'after_widget' 	=> '</div>',
				'before_title' 	=> '<h4><span>',
		...
		function boldthemes_admin_init() {
		...
		function boldthemes_load_admin_style() {
			wp_enqueue_style( 'hotelcalifornia-editor-style', get_parent_theme_file_uri( 'editor-style.css' ) );
		}
		...
		function boldthemes_enqueue_scripts_styles() {
			...
			$boldthemes_crush_vars_def = array( 'alternateColor', 'accentColor', 'logoHeight', 'logoIndent', 'bodyFont', 'menuFont', 'headingFont', 'headingSuperTitleFont', 'headingSubTitleFont' );	
			...
			$logo_indent = boldthemes_get_option( 'logo_indent' );
			$logo_height = boldthemes_get_option( 'logo_height' );
			...
			if ( $logo_indent != '' ) {
				$boldthemes_crush_vars['logoIndent'] = $logo_indent;
			}
			
			if ( $logo_height != '' ) {
				$boldthemes_crush_vars['logoHeight'] = $logo_height;
			}
			...
			// custom theme css
			wp_enqueue_style( 'hotelcalifornia-style', get_template_directory_uri() . '/style.css', array(), false );
			
			// custom magnific popup css
			wp_enqueue_style( 'hotelcalifornia-magnific-popup', get_template_directory_uri() . '/magnific-popup.css', array(), false );
			
			wp_enqueue_script( 'slick.min', get_template_directory_uri() . '/js/slick.min.js', array( 'jquery' ), '', false );
			wp_enqueue_script( 'jquery.magnific-popup.min', get_template_directory_uri() . '/js/jquery.magnific-popup.min.js', array( 'jquery' ), false, false );
			if ( ! wp_is_mobile() ) wp_enqueue_script( 'iscroll', get_template_directory_uri() . '/js/iscroll.js', array( 'jquery' ), false, false );
			wp_enqueue_script( 'fancySelect', get_template_directory_uri() . '/js/fancySelect.js', array( 'jquery' ), false, false );			
			...
			wp_enqueue_script( 'hotelcalifornia-header-misc', get_template_directory_uri() . '/js/header.misc.js', array( 'jquery' ), false, false );
			wp_add_inline_script( 'hotelcalifornia-header-misc', HotelCaliforniaTheme::boldthemes_set_global_uri(), 'before' );
			// custom footer related js
			wp_enqueue_script( 'hotelcalifornia-footer-misc', get_template_directory_uri() . '/js/footer.misc.js', array( 'jquery' ), false, true );			
			// custom miscellaneous js			
			wp_enqueue_script( 'hotelcalifornia-misc', get_template_directory_uri() . '/js/misc.js', array( 'jquery' ), false, false );
			// custom tile hover effect js	
			wp_enqueue_script( 'hotelcalifornia-dir-hover', get_template_directory_uri() . '/js/dir.hover.js', array( 'jquery' ), false, false );
			// custom slider js
			wp_enqueue_script( 'hotelcalifornia-sliders', get_template_directory_uri() . '/js/sliders.js', array( 'jquery' ), false, false );
			...
			if ( file_exists( get_template_directory() . '/css-override.php' ) ) {
				...
				if ( count( $boldthemes_crush_vars ) > 0 ) wp_add_inline_style( 'hotelcalifornia-style', $css_override );
			}
		...
		function boldthemes_load_fonts() {
			...
			if ( $body_font != 'no_change' ) {
				 ...
				 */
				if ( 'off' !== _x( 'on', 'Playfair Display font: on or off', 'hotelcalifornia' ) ) {
					$font_families[] = 'Playfair Display' . ':100,200,300,400,500,600,700,800,900,100italic,200italic,300italic,400italic,500italic,600italic,700italic,800italic,900italic';
				}
			}
			...
			if ( $menu_font != 'no_change' ) {
				...
				by chosen font(s), translate this to 'off'. Do not translate into your own language.
				 */
				if ( 'off' !== _x( 'on', 'Lato font: on or off', 'hotelcalifornia' ) ) {
			...
			if ( count( $font_families ) > 0  ) {
				...
				wp_enqueue_style( 'hotelcalifornia-fonts', $font_url, array(), '1.0.0' );
			}
		}
		...
		function boldthemes_theme_register_required_plugins() {
			...
			$plugins = array(
				...
				array(
					'name'               => 'Hotel California', // The plugin name.
					'slug'               => 'hotelcalifornia', // The plugin slug (typically the folder name).
					'source'             => get_template_directory() . '/plugins/hotelcalifornia.zip', // The plugin source.
					'required'           => true, // If false, the plugin is only 'recommended' instead of required.
					'version'            => '1.1.8', // E.g. 1.0.0. If set, the active plugin must be this version or higher.
				...
				array(
					'name'               => esc_html__( 'Bold Builder', 'hotelcalifornia' ), // The plugin name.
			...
			$config = array(
				...
				'strings'      => array(
					'page_title'                      => esc_html__( 'Install Required Plugins', 'hotelcalifornia' ),
					'menu_title'                      => esc_html__( 'Install Plugins', 'hotelcalifornia' ),
					'installing'                      => esc_html__( 'Installing Plugin: %s', 'hotelcalifornia' ), // %s = plugin name.
					'oops'                            => esc_html__( 'Something went wrong with the plugin API.', 'hotelcalifornia' ),
					'notice_can_install_required'     => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'hotelcalifornia' ), // %1$s = plugin name(s).
					'notice_can_install_recommended'  => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'hotelcalifornia' ), // %1$s = plugin name(s).
					'notice_cannot_install'           => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'hotelcalifornia' ), // %1$s = plugin name(s).
					'notice_can_activate_required'    => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'hotelcalifornia' ), // %1$s = plugin name(s).
					'notice_can_activate_recommended' => _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'hotelcalifornia' ), // %1$s = plugin name(s).
					'notice_cannot_activate'          => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'hotelcalifornia' ), // %1$s = plugin name(s).
					'notice_ask_to_update'            => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'hotelcalifornia' ), // %1$s = plugin name(s).
					'notice_cannot_update'            => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'hotelcalifornia' ), // %1$s = plugin name(s).
					'install_link'                    => _n_noop( 'Begin installing plugin', 'Begin installing plugins', 'hotelcalifornia' ),
					'activate_link'                   => _n_noop( 'Begin activating plugin', 'Begin activating plugins', 'hotelcalifornia' ),
					'return'                          => esc_html__( 'Return to Required Plugins Installer', 'hotelcalifornia' ),
					'plugin_activated'                => esc_html__( 'Plugin activated successfully.', 'hotelcalifornia' ),
					'complete'                        => esc_html__( 'All plugins installed and activated successfully. %s', 'hotelcalifornia' ), // %s = dashboard link.
		...
		function boldthemes_search_form( $form ) {
			...
			<div class="btSearchInner" role="search">
				<div class="btSearchInnerContent">
					<form action="' . home_url( '/' ) . '" method="get"><input type="text" name="s" placeholder="' . esc_attr( esc_html__( 'Looking for...', 'hotelcalifornia' ) ) . '" class="untouched">
		...
		function boldthemes_wp_video_shortcode_library() {
			...
			wp_enqueue_script( 'hotelcalifornia-video-shortcode', get_template_directory_uri() . '/js/video_shortcode.js', array( 'mediaelement' ), '', true );
		...
		function boldthemes_wp_audio_shortcode_library() {
			wp_enqueue_style( 'wp-mediaelement' );
			wp_enqueue_script( 'hotelcalifornia-audio-shortcode', get_template_directory_uri() . '/js/audio_shortcode.js', array( 'mediaelement' ), '', true );
			return 'boldthemes_mejs';
		}
		 ...
		 * @return string
		 */
		function boldthemes_title( $title, $short = true ) {
			if ( empty( $title ) && ( is_home() || is_front_page() ) ) {
				$title = esc_html__( 'Home', 'hotelcalifornia' );
			}
	...
	$boldthemes_theme = new HotelCaliforniaTheme();
...
if ( ! function_exists( 'boldthemes_pagination' ) ) {
	function boldthemes_pagination() {
		...
		$prev = get_previous_posts_link( esc_html__( 'Newer Posts', 'hotelcalifornia' ) );
		$next = get_next_posts_link( esc_html__( 'Older Posts', 'hotelcalifornia' ) );
...
if ( ! class_exists( 'RWMB_BoldThemesText_Field' ) && class_exists( 'RWMB_Field' ) ) {
	class RWMB_BoldThemesText_Field extends RWMB_Field {
		...
		static function admin_enqueue_scripts() {			
			wp_enqueue_script( 
				'hotelcalifornia-boldthemes-text',
...
if ( ! function_exists( 'boldthemes_theme_comment' ) ) {
	function boldthemes_theme_comment( $comment, $args, $depth ) {
		...
		<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
			<p><?php esc_html_e( 'Pingback:', 'hotelcalifornia' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( esc_html__( '(Edit)', 'hotelcalifornia' ), '<span class="edit-link">', '</span>' ); ?></p>
		<?php
				break;
			default :
				...
				<div class="commentTxt">
					<div class="vcard divider">
						<?php
							...
							echo '<p class="posted">' . sprintf( esc_html__( '%1$s at %2$s', 'hotelcalifornia' ), get_comment_date(), get_comment_time() ) . '</p>';
							if ( $rating > 0 && get_option( 'woocommerce_enable_review_rating' ) == 'yes' ) { ?>
								<div itemprop="reviewRating" itemscope itemtype="http://schema.org/Rating" class="star-rating" title="<?php echo sprintf( esc_html__( 'Rated %d out of 5', 'hotelcalifornia' ), $rating ) ?>">
									<span style="width:<?php echo ( esc_html( $rating ) / 5 ) * 100; ?>%"><strong itemprop="ratingValue"><?php echo wp_kses_post( $rating ); ?></strong> <?php esc_html_e( 'out of 5', 'hotelcalifornia' ); ?></span>
								</div>
							<?php }
						?>
					</div>
					...
					<?php if ( '0' == $comment->comment_approved ) : ?>
						<p class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'hotelcalifornia' ); ?></p>
					<?php endif; ?>
					...
					<div class="comment">
						...
						if ( comments_open() ) {
							echo '<p class="reply">';
								comment_reply_link( array_merge( $args, array( 'reply_text' => esc_html__( 'Reply', 'hotelcalifornia' ), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) );
							echo '</p>';
						}
						edit_comment_link( esc_html__( 'Edit', 'hotelcalifornia' ), '<p class="edit-link">', '</p>' ); ?>
					</div>
				</div>
...
if(isset($_COOKIE['_icl_current_language']) && defined('ICL_SITEPRESS_VERSION')){
        global $sitepress;
...
if ( ! function_exists( 'boldthemes_get_posts_array_item' ) ) {
	function boldthemes_get_posts_array_item( $post_type = 'blog', $post_id, $post_author ) {
		...
		$post_data['date'] = date_i18n( HotelCaliforniaTheme::$boldthemes_date_format, strtotime( get_the_time( 'Y-m-d', $post_id ) ) );
...
if ( ! function_exists( 'boldthemes_set_override' ) ) {
	function boldthemes_set_override() {
		...
		HotelCaliforniaTheme::$boldthemes_page_options = array();
		...
		if ( ! is_404() ) {
			...
			if ( is_singular( 'post' ) && isset( HotelCaliforniaTheme::$boldthemes_options['blog_settings_page_slug'] ) && isset( HotelCaliforniaTheme::$boldthemes_options['blog_settings_page_slug'] ) && HotelCaliforniaTheme::$boldthemes_options['blog_settings_page_slug'] != '' ) {
				$tmp_boldthemes_page_options1 = boldthemes_rwmb_meta( BoldThemesPFX . '_override', array(), boldthemes_get_id_by_slug( HotelCaliforniaTheme::$boldthemes_options['blog_settings_page_slug'] ) );
			} 
			if ( ( is_post_type_archive( 'portfolio' ) || is_singular( 'portfolio' ) ) && isset( HotelCaliforniaTheme::$boldthemes_options['pf_settings_page_slug'] ) && HotelCaliforniaTheme::$boldthemes_options['pf_settings_page_slug'] != '' ) {
				$tmp_boldthemes_page_options1 = boldthemes_rwmb_meta( BoldThemesPFX . '_override', array(), boldthemes_get_id_by_slug( HotelCaliforniaTheme::$boldthemes_options['pf_settings_page_slug'] ) );
			} 
			...
			if ( function_exists( 'is_product' ) && is_product() && isset( HotelCaliforniaTheme::$boldthemes_options['shop_settings_page_slug'] ) && HotelCaliforniaTheme::$boldthemes_options['shop_settings_page_slug'] != '' ) {
				$tmp_boldthemes_page_options1 = boldthemes_rwmb_meta( BoldThemesPFX . '_override', array(), boldthemes_get_id_by_slug( HotelCaliforniaTheme::$boldthemes_options['shop_settings_page_slug'] ) );
			}
			...
			foreach ( $tmp_boldthemes_page_options as $key => $value ) {
				HotelCaliforniaTheme::$boldthemes_page_options[ $key ] = $value;
			}
		}
	}
}
...
if ( ! function_exists( 'boldthemes_get_body_class' ) ) {
	function boldthemes_get_body_class() {
		...
		HotelCaliforniaTheme::$boldthemes_sidebar = boldthemes_get_option( 'sidebar' );

		if ( ! ( ( HotelCaliforniaTheme::$boldthemes_sidebar == 'left' || HotelCaliforniaTheme::$boldthemes_sidebar == 'right' ) && ! is_404() ) ) {
			HotelCaliforniaTheme::$boldthemes_has_sidebar = false;
			$extra_class[] = 'btNoSidebar';
		} else {
			HotelCaliforniaTheme::$boldthemes_has_sidebar = true;
			if ( HotelCaliforniaTheme::$boldthemes_sidebar == 'left' ) {
